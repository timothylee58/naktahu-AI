...
| Embeddings | **Dual, ILMU-first** (user decision 2026-09-23, migration 051): `embedding_ilmu` vector(4096) via `rag_node._embed_ilmu` → `hybrid_search_ilmu`; fallback `embedding` vector(1536, OpenAI `text-embedding-3-small`, what the corpus was originally built with) via `rag_node._embed` → `hybrid_search`. Never compare one model's vector against the other's column — see `llm_client.py` |
