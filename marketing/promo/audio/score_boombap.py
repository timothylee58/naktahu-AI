"""Boom-bap score (soulful, J. Cole / Kendrick / JID-type) for the feature showcase.

64.29 BPM half-time: one beat = 28 frames at 30fps, one bar = 112 frames = two
visual scenes, so every scene cut lands on the kick or the snare.
"""
import json
import sys
import wave
...
def rhodes(notes, dur):
    ...
    for m in notes:
        f = mtof(m)
        mod = np.sin(2 * np.pi * f * 14 * t) * np.exp(-t * 9) * 1.4
...
PROG = [  # Fm9 - Bbm9 - Ebmaj9 - Db maj7 / C7#9 turnaround
    ([41], [56, 60, 63, 67]),
    ([46], [56, 61, 65, 68]),
    ([39], [55, 58, 62, 65]),
    ([37, 36], [56, 60, 63, 65]),
]
CHORDS = ["Fm9", "Bbm9", "Ebmaj9", "Dbmaj7"] * 3
DROP_OUT = (504, 560)  # drums out for the "12 agents" breakdown scene
END = 1064
...
def drums_on(fr):
    return 112 <= fr < END and not (DROP_OUT[0] <= fr < DROP_OUT[1])
...
for b in range(NBARS):
    ...
    if f0 >= TOTAL_FRAMES:
        break
    dur = HBAR / FPS
    if b % 4 == 3:  # split bar: Dbmaj7 then C7#9
        add(rhodes(voicing, dur / 2), fs(f0), 1.0, pan=-0.15, bus=keys)
        add(rhodes([55, 58, 63, 64], dur / 2), fs(f0 + HBAR / 2), 1.0, pan=-0.15, bus=keys)
    else:
        add(rhodes(voicing, dur * 0.55), fs(f0), 1.0, pan=-0.15, bus=keys)
        add(rhodes(voicing[1:], dur * 0.3), fs(f0 + swung(10)), 0.6, pan=-0.15, bus=keys)
...
keys[:, : fs(112)] = np.vstack([lp(keys[0, : fs(112)], 900), lp(keys[1, : fs(112)], 900)])
...
add(rhodes([53, 56, 60, 63, 67], 2.2), fs(END), 0.9, verb=0.4)
