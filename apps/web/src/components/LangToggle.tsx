  ...
  return (
    <div ref={ref} className="relative">
      <motion.button
        ...
        aria-expanded={open}
        className={`flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-semibold transition-colors ${buttonBase}`}
      >
        {/* Globe icon */}
        ...
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className={`w-3 h-3 opacity-50 transition-transform duration-200 ${open ? 'rotate-180' : ''}`}>
          ...
          >
            {OPTIONS.map((opt) => (
              ...
              >
                ...
                <span className={`text-xs ${opt.value === locale ? '' : 'opacity-50'}`}>{opt.label}</span>
              </button>
            ))}
          </motion.div>
        )}
