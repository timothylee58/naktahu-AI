      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        <section className="flex flex-col items-center justify-center flex-1 text-center px-6 py-24 gap-8">
          ...
          <motion.h1
            ...
            initial="hidden"
            animate="show"
            className="text-4xl sm:text-6xl font-bold leading-tight max-w-3xl tracking-tight locale-text-balance"
          >
            {(() => {
          ...
          <motion.div custom={4} variants={fadeUp} initial="hidden" animate="show">
            <Link
              href="/chat"
              className="inline-flex items-center gap-2 bg-[#2563EB] hover:bg-blue-500 transition-colors text-white font-semibold px-8 py-3.5 rounded-full text-base shadow-lg shadow-blue-900/40 locale-nowrap"
            >
              {t('landing.hero.cta')}
              <svg
                xmlns="http://www.w3.org/2000/svg"
        ...
        >
          <h2 className="text-center text-2xl font-bold mb-12 text-zinc-200 locale-text-balance">
        ...
        >
          <h2 className="text-2xl font-bold text-zinc-200 locale-text-balance">{t('landing.domains.title')}</h2>
          <div className="flex flex-wrap justify-center gap-3 max-w-2xl">
            {DOMAINS.map((d, i) => (
              <motion.span
                ...
                className="border border-[#2563EB]/40 text-[#2563EB] rounded-full px-4 py-1.5 text-sm font-medium bg-[#2563EB]/10 locale-nowrap"
              >
                {t(`domain.${d.key}`)}
              </motion.span>
            ))}
          ...
          <div className="flex flex-col items-end gap-1 text-right">
            <div className="flex items-center gap-3">
              <a
                ...
                className="hover:text-white transition-colors locale-nowrap"
              >
                {t('landing.footer.github')} ↗
              </a>
              <Link href="/privacy" className="hover:text-white transition-colors locale-nowrap">
                {t('footer.privacy')}
              </Link>
            </div>
            <span className="text-xs max-w-xs text-right locale-text-balance">
