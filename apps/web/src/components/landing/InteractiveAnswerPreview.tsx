...
// Deliberately doesn't fabricate a specific hotline phone number (those
// drift and this repo has no verified-current source for one); the
// "ministry" labels/domains here mirror real citation shape ({ministry,
...
export function InteractiveAnswerPreview({ isDark = true }: InteractiveAnswerPreviewProps) {
  const { t } = useI18n();
  const answer = t('landing.preview.demo_answer');
  const [visibleChars, setVisibleChars] = useState(0);
  const [started, setStarted] = useState(false);
  const containerRef = useRef<HTMLDivElement>(null);

  // Only start the typing simulation once the card scrolls into view, and
  // only once — replaying it every scroll would be distracting rather than
  // demonstrative.
  useEffect(() => {
  ...
  return (
    <motion.div
      ...
      className={`max-w-2xl mx-auto w-full rounded-2xl border overflow-hidden ${cardClass}`}
    >
