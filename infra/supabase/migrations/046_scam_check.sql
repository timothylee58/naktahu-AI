...
CREATE TABLE IF NOT EXISTS official_gov_domains (
  ...
  common_scam_patterns  text[],                 -- e.g. ARRAY['fake refund SMS', 'fake summons link']
...
DROP POLICY IF EXISTS "official_gov_domains_public_read" ON official_gov_domains;
...
-- Uses ARRAY[...] rather than the '{...}' curly-brace literal form — the
-- curly-brace form treats double quotes as its own quoting syntax, so any
-- element containing a literal " (several below quote a scam SMS's actual
-- wording) breaks parsing ("malformed array literal" / "incorrectly quoted
-- array element"). ARRAY[...] takes plain, ordinarily-quoted SQL strings
-- with no such ambiguity. Confirmed by actually running this migration.
INSERT INTO official_gov_domains (institution_name, institution_name_bm, domain, agency_type, category, common_scam_patterns, official_contact) VALUES
  ('Inland Revenue Board (LHDN)', 'Lembaga Hasil Dalam Negeri', 'hasil.gov.my', 'government', 'tax',
    ARRAY['fake tax refund SMS', 'fake e-Filing login link', 'fake "outstanding tax" WhatsApp message'], 'https://www.hasil.gov.my'),
  ('LHDN e-Filing', 'e-Filing LHDN', 'ezhasil.gov.my', 'government', 'tax',
    ARRAY['fake e-Filing login page', 'phishing link mimicking ezhasil'], 'https://ezhasil.gov.my'),
  ('Road Transport Department (JPJ)', 'Jabatan Pengangkutan Jalan', 'jpj.gov.my', 'government', 'government',
    ARRAY['fake summons/saman payment link', 'fake licence renewal SMS'], 'https://www.jpj.gov.my'),
  ('Employees Provident Fund (EPF/KWSP)', 'Kumpulan Wang Simpanan Pekerja', 'kwsp.gov.my', 'statutory_body', 'epf',
    ARRAY['fake EPF withdrawal approval link', 'fake i-Akaun login page', 'fake "your KWSP account is locked" call'], 'https://www.kwsp.gov.my'),
  ('Social Security Organisation (SOCSO/PERKESO)', 'Pertubuhan Keselamatan Sosial', 'perkeso.gov.my', 'statutory_body', 'epf',
    ARRAY['fake SOCSO claim approval SMS', 'fake compensation payout link'], 'https://www.perkeso.gov.my'),
  ('Royal Malaysia Police (PDRM)', 'Polis DiRaja Malaysia', 'rmp.gov.my', 'government', 'government',
    ARRAY['fake police report/summons call', '"your IC was used in a crime" scam call'], 'https://www.rmp.gov.my'),
  ('Bank Negara Malaysia', 'Bank Negara Malaysia', 'bnm.gov.my', 'government', 'finance',
    ARRAY['fake BNM "your bank account is under investigation" call', 'fake BNM licensing verification link'], 'https://www.bnm.gov.my'),
  ('National Registration Department (JPN)', 'Jabatan Pendaftaran Negara', 'jpn.gov.my', 'government', 'government',
    ARRAY['fake MyKad renewal link', 'fake "IC suspended" SMS'], 'https://www.jpn.gov.my'),
  ('Immigration Department of Malaysia', 'Jabatan Imigresen Malaysia', 'imi.gov.my', 'government', 'immigration',
    ARRAY['fake passport renewal link', 'fake visa/permit fine payment SMS'], 'https://www.imi.gov.my'),
  ('Companies Commission of Malaysia (SSM)', 'Suruhanjaya Syarikat Malaysia', 'ssm.com.my', 'statutory_body', 'business',
    ARRAY['fake company registration renewal invoice', 'fake "your SSM registration will be suspended" email'], 'https://www.ssm.com.my'),
  ('Pos Malaysia', 'Pos Malaysia', 'pos.com.my', 'postal', 'government',
    ARRAY['fake parcel customs-fee payment link', 'fake "parcel held" SMS'], 'https://www.pos.com.my'),
  ('Ministry of Health (MOH) / MySejahtera', 'Kementerian Kesihatan Malaysia', 'moh.gov.my', 'government', 'healthcare',
    ARRAY['fake MySejahtera update link', 'fake vaccination certificate scam'], 'https://www.moh.gov.my'),
  ('Maybank', 'Maybank', 'maybank2u.com.my', 'bank', 'finance',
    ARRAY['fake Maybank2u login page', 'fake "your account is frozen" SMS'], 'https://www.maybank2u.com.my'),
  ('CIMB Bank', 'CIMB Bank', 'cimbclicks.com.my', 'bank', 'finance',
    ARRAY['fake CIMB Clicks login page', 'fake OTP request call'], 'https://www.cimbclicks.com.my'),
  ('Public Bank', 'Public Bank', 'pbebank.com', 'bank', 'finance',
    ARRAY['fake PBe login page', 'fake "suspicious transaction" SMS'], 'https://www.pbebank.com');
